...
class ActivityBudgetReportController extends Controller
{
    ...
    public function index(){
    	return view('projectreport.activity-budget-report.index');
    }
    ...
    public function filterSearch(Request $request)
    {
       ...
       if ($request->has('project_status')) {
        ...
        $root_url = url('/');
        $baseurl = $root_url.'/report/activity-budget-report/';
        
        foreach($lists as $key => $list){
            foreach($list->work_detail as $workDetail)
            {
                $html .= '<tr>
                        <td>'. ++$key .'</td>
                        <td>'. $workDetail->activityMain->name   .'</td>
